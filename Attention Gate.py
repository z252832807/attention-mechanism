...
class Attention_block(nn.Module):
    ...
    def forward(self, g, x):  # 需要输入特征层 g & x
        g1 = self.W_g(g)  # 卷积层 out_channel = F_int
        x1 = self.W_x(x)  # 卷积层 out_channel = F_int
        psi = self.relu(g1 + x1)  # 线性相加后一起送入Sigmoid
        psi = self.psi(psi)
...
class AttU_Net(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, scale_factor=1):
        ...
        self.Up5 = up_conv(ch_in=filters[4], ch_out=filters[3])  # in = 1024 , out = 512
    ...
    def forward(self, x):
        ...
        x4 = self.Maxpool(x3)
        x4 = self.Conv4(x4)  # 4 512 56 56

        x5 = self.Maxpool(x4)
        x5 = self.Conv5(x5)  # 4 1024 28 28

        # decoding + concat path
        d5 = self.Up5(x5)  # 4 512 56 56
